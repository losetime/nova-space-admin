...
export class PushSchedulerService {
  ...
  async triggerManualPush(userId: string): Promise<boolean> {
    ...
    try {
      const content = await this.digestService.generateDigestContent(
        subscription as any,
      );
  ...
  async handleDailyPush() {
    ...
    try {
      ...
      const needPushUsers = this.filterTodayUnpushed(subscriptions);
      ...
      this.logger.log(
        `推送完成：成功 ${results.success}, 失败 ${results.failed}`,
      );
    } catch (error) {
  ...
  async handleRetryFailedPush() {
    ...
    try {
      ...
      this.logger.log(
        `重试完成：成功 ${results.success}, 失败 ${results.failed}`,
      );
    } catch (error) {
  ...
  private filterTodayUnpushed(subscriptions: any[]): any[] {
  ...
  private async batchPush(
    ...
    const batchSize =
      this.configService.get<number>("app.push.batchSize") || 10;
    const batchInterval =
      this.configService.get<number>("app.push.batchInterval") || 1;
    ...
    for (let i = 0; i < subscriptions.length; i += batchSize) {
      ...
      if (i + batchSize < subscriptions.length) {
        await new Promise((resolve) =>
          setTimeout(resolve, batchInterval * 1000),
        );
      }
    }
  ...
  private async recordPushResult(
    ...
    await this.db.insert(pushRecords).values({
      ...
      subject: `星揽 每日资讯 - ${new Date().toLocaleDateString("zh-CN")}`,
